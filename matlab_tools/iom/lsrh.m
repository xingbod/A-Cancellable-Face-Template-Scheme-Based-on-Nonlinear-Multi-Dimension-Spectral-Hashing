function [code, model] = lsrh(data, opts, model)
%% Source code for Cross-modal Ranking Subspace Hashing (CMRSH)
% Code author: Kai Li. Contact: kaili@cs.ucf.edu or kailee.ucf@gmail.com
% Please cite this paper:
% "Cross-modal Hashing Through Ranking Subspace Learning", ICME 2016
% Kai Li, Guojun Qi, Jun Ye, Kien A Hua.

% If no model is provided, learn the model first
if nargin == 2
    model = trainCMRSH(data, opts);
end

%%=========================================================================
function model = trainCMRSH(data, opts)
X = data.X;
Y = data.Y;
S = data.S;
