function [DIR,mAP, FAR, thresholds] = OpenSetROC2( score, galLabels, probLabels, farPoints, rankPoints,top_k )
...
T1 = bsxfun(@ge, genScore, thresholds); % compare genuine scores to the decision thresholds // this means I can detect this subject
T2 = bsxfun(@le, probRanks, rankPoints); % compare the genuine probe matching ranks to the interested rank points // is under the ranks within my ranks
T = bsxfun(@and, reshape(T1, Ngen, 1, []), T2); % detection and identification should be both satisfied

%%
% here we cal the mAP here the gen score is S  gen label similarity is M 
distanceS=score(:, genProbIndex)';
labelL=M';
[Ntest, Ndb] = size(distanceS);

% mAP and topM preision
[~, Id] = sort(distanceS, 2,'descend');                
lid = sub2ind(size(distanceS), repmat(1:Ntest, 1, Ndb), Id(:)');
Stmp = reshape(labelL(lid), Ntest, Ndb);
Distancetmp = reshape(distanceS(lid), Ntest, Ndb);
Scumsum = cumsum(Stmp, 2);
P =  Scumsum ./ repmat(1:Ndb, Ntest, 1);% j th precision
R =  Scumsum ./ sum(labelL,2) ;% j th recall R(0)=0;
R_10 = R(:, 1:top_k);
for ri=1:top_k
    if ri==1
         R_10(:,ri)=R(:,ri) - 0;
...
for tindex=1:length(thresholds)
    P_Threshold=P.*(Distancetmp>=thresholds(tindex));
    
    P_10 = P_Threshold(:, 1:top_k);
   
    mAP(tindex) =mean(sum(P_10.*R_10, 2));
end
